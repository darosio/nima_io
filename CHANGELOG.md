...
- Bioio to read images as ArrayData with Metadata

### 🐛 Bug Fixes

- Tutorial run and warnings

### 📚 Documentation

- Mention maven dep
- Drop RTD
- *(fix)* Misconfigured conf.py

### 🦾 Build

- *(deps)* Drop bfio
- *(deps)* Drop pyometiff
- *(deps)* Bump numpy from \<=2.1.0 to 2.4.2
- *(deps)* Bump sphinx from \<=8.0.2 to \<=8.2.3
- *(deps)* Drop jpype1
- *(deps)* Bump matplotlib from \<=3.9.2 to \<=3.10.6
- *(deps)* Drop scyjava
- *(deps)* Bump scikit-image from \<=0.24.0 to \<=0.25.2
- *(deps)* Bump click from \<=8.1.7 to 8.3.1
- *(deps)* Drop nbsphinx
- *(deps)* Drop lxml
- *(deps)* Bump bioio-bioformats from \<=1.0.0 to 1.3.2
- *(deps)* Drop readlif
- *(deps)* Drop xmltodict
- *(deps)* Drop dask-image
- Add Python 3.13
- Drop Python 3.10
- *(deps)* Bump python Docker tag to v3.14 (#603)
- *(deps)* Lock file maintenance
- Manual cruft update
- *(deps)* Bump xarray from 2026.1.0 to 2026.2.0
